...
import {  years_data } from '../data/alldata';
import { useSearchParams } from 'react-router-dom';
import { getObjectByEndYear } from '../services/operations';
...
const EndYearObject = () => {
  ...
  const dropdownOptions = years_data;
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const data = await getObjectByEndYear(selectedOption?.name);
  ...
  useEffect(() => {
    ...
    if (nameFromParams) {
      // Find the region in dropdownOptions based on the name from URL
      const selectedRegion = dropdownOptions.find(
  ...
  return (
    <div className='container mx-auto flex flex-col w-[90%]'>
       <h1 className='text-3xl font-bold text-richblack-800'>Search by End Year</h1>
