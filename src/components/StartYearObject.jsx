...
import {  start_data } from '../data/alldata';
    ...
    try {
      ...
      if(data){
        ...
        const filteredObjects = data.filter(obj => {
            ...
            const publishedDate = new Date(obj.published);
          
            const publishedYear = publishedDate.getFullYear();
           
            return obj.start_year && obj.start_year === publishedYear;
          });
          setFilteredObjects(filteredObjects);
      }
      else{
        console.error("do not recived data")
      }
    } catch (error) {
...
export default StartYearObject;
