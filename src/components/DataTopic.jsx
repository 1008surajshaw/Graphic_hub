...
import { clusteredDatatopic } from '../data/alldata'
import { useSearchParams } from 'react-router-dom';
import { getObjectBySector, getObjectByTopic } from "../services/operations"
...
const DataTopic = () => {
    ...
    const dropdownOptions = clusteredDatatopic;
  ...
  const [flag,setFlag] = useState(false);
    ...
    function capitalizeWords(input) {
      ...
      const handleSearchChange = (event) => {
        ...
        const selectedRegion = dropdownOptions.find((topic) =>
          topic.contains.includes(formattedSearchTerm)
        );
      
        if (selectedRegion && selectedRegion.contains) {
          // Find the exact match in the name array
          const exactMatch = selectedRegion.contains.find(
      ...
      const fetchData = async () =>{
        setLoading(true);
        try {
            // Call your asynchronous function to get data
            
            const data = await getObjectByTopic(selectedOption);
  ...
  return (
    <div className='container mx-auto flex flex-col w-[90%]'>
      <h1 className='text-3xl font-bold text-richblack-800'>Search by Topic</h1>
    ...
    <div className='flex'>
     ...
     <select onChange={handleDropdownChange} value={selectedOption || ''}  className='w-full md:w-64 mt-4 px-4 py-2 rounded-md border focus:outline-none focus:border-blue-500 mr-4'>
      {dropdownOptions.map((option) => (
        ...
        {option.contains.map((nameOption, index) => (
